...
window.onload = function () {
    ...
    calendarParams = {
        ...
        maximumDate: "",
        dates: {
            min: onloadMoment.format("DD-MM-YYYY"),
            max: moment("23-02-2020", "DD-MM-YYYY").format("DD-MM-YYYY")
        },
        datepicker: {
            calendarIds: "cal"
        },
...
function fareCalendarGenerator(calendarParams) {
    ...
    let fareCalendar = renderCalendar(calendarParams);
...
function renderCalendar(calendarParams) {

    let startMomentDate = calendarParams.dates.min;
    let referredBy = calendarParams.referredBy;
    let maxDate = calendarParams.dates.max;


    let temporaryMoment = moment(startMomentDate, "DD-MM-YYYY");
    ...
    for (; c <= 6; c++) {
        ...
        tdClassAdder(count, maxDate, td, temporaryMoment);
...
function nextMonth() {
    ...
    calendarParams.startDate = nextMonth.format("DD-MM-YYYY");
    fareCalendarGenerator(calendarParams);
}
...
function previousMonth() {
    ...
    calendarParams.startDate = previousMoment.format("DD-MM-YYYY");
    fareCalendarGenerator(calendarParams);
}
...
function tdClassAdder(count, maxDate, td, temporaryLoopMoment) {

    let dateDisabled = " date-disabled";
    let allowedDate = " date-allowed";
    let todayDate = " today-date";

    if (temporaryLoopMoment.month() === maxDate.month() &&
        temporaryLoopMoment.year() === maxDate.year() &&
        count === maxDate.date()) {

        td.className += todayDate;

    } else if (temporaryLoopMoment.isBefore(maxDate)) {
